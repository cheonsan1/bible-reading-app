// 시스템 소스 다운로드 (관리자 전용)
    ...
    // 명예의전당
    function filterJujuDok(dok) {
        state.selectedDokTab = dok;
        document.querySelectorAll('#page-juju_records button').forEach((btn, idx) => {
            if (idx === dok - 1) btn.className = "px-4 py-2 rounded-full border text-xs font-semibold bg-purple-100 border-purple-300 text-purple-800";
            else btn.className = "px-4 py-2 rounded-full border text-xs font-semibold bg-white border-stone-200 text-stone-600";
        });
        applyStateToDOM();
    }
